...
- [x] 2.2 Implement batch workload capture with normalized SQL text, bind metadata, Oracle metrics, and Oracle plan rows using `python-oracledb`.
- [ ] 2.3 Implement stream-mode watermark polling and schema filtering, with tests that verify append-only workload behavior.
...
- [ ] 3.3 Implement typed bind literalization, unsupported bind classification, and explicit replay skip statuses for statements that cannot be represented safely.
...
- [ ] 4.3 Define the replay backend interface and add a placeholder integration seam for a future OBCI-backed helper without changing artifact schemas.
...
- [ ] 5.3 Add golden-file or fixture-based tests for report ranking, rule matching, and error classification outputs.
